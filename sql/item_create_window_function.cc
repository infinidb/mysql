...
int item_window_function_create_init()
{
	...
	if (hash_init(& native_window_functions_hash,
	              system_charset_info,
	              array_elements(func_array),
	              0,
	              0,
	              (hash_get_key) get_native_window_fct_hash_key,
	...
	if (hash_init(& native_window_functions_hash_nulls,
	              system_charset_info,
...
void item_window_function_create_cleanup()
{
	...
	hash_free(& native_window_functions_hash);
	hash_free(& native_window_functions_hash_nulls);
	DBUG_VOID_RETURN;
}
...
find_native_window_function_builder(THD *thd, LEX_STRING name)
{
	...
	/* Thread safe */
	func= (Native_window_func_registry*) hash_search(& native_window_functions_hash,
	                                                 (uchar*) name.str,
	                                                 name.length);
...
find_native_window_function_builder_nulls(THD *thd, LEX_STRING name)
{
	...
	/* Thread safe */
	func= (Native_window_func_registry*) hash_search(& native_window_functions_hash_nulls,
	                                                 (uchar*) name.str,
	                                                  name.length);
