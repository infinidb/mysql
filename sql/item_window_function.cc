...
void Item_func_window_avg::fix_length_and_dec ()
{
	...
	if (hybrid_type == DECIMAL_RESULT)
	{
		...
		decimals= std::min(static_cast<int>(args[0]->decimals + prec_increment), DECIMAL_MAX_SCALE);
		max_length= my_decimal_precision_to_length_no_truncation(precision,
		                                                         decimals,
		                                                         unsigned_flag);
	}
	...
	{
		decimals= std::min(static_cast<int>(args[0]->decimals + prec_increment), NOT_FIXED_DEC);
...
void Item_func_window_stats::fix_length_and_dec()
{
	...
	*/
	...
	case REAL_RESULT:
	case STRING_RESULT:
		decimals= std::min(args[0]->decimals + 4, NOT_FIXED_DEC);
		break;
	case INT_RESULT:
	case DECIMAL_RESULT:
	{
...
bool Item_func_window_median::fix_fields(THD* thd, Item** ref)
{
	...
	SQL_I_List<ORDER>* orders = new SQL_I_List<ORDER>();
	orders->elements= 0;
	orders->first= 0;
...
bool Item_func_window::get_time(st_mysql_time*)
{
	return FALSE;
}

bool Item_func_window::get_date(st_mysql_time*, uint)
{
	return FALSE;
}
